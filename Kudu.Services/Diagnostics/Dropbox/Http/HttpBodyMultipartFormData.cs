    ...
    public class HttpBodyMultipartFormData
    {
        ...
        public Byte[] CreateBodyData()
        {
            ...
            foreach (var fd in _FormDataList)
            {
                ...
                if (String.IsNullOrWhiteSpace(fd.ContentType) == false)
                {
                    sb.AppendFormat("Content-Type: {0};", fd.ContentType);
                    if (String.IsNullOrWhiteSpace(fd.Charset) == false)
                    {
                ...
                if (String.IsNullOrWhiteSpace(fd.ContentTransferEncoding) == false)
                {
